...
Transform.trelloUsernameToId = function (username) {
  ...
  let ownerInfo = memberlist.find(function (member) {
    return member.username.includes(username.toLowerCase().slice(1));
  });
  return ownerInfo.id;
};
...
Transform.trelloIdToUsername = function (id) {
  ...
  let username = memberlist.find(function (member) {
    return member.id.includes(id);
  }).username;
  return username;
};

...
pairs.forEach(function (pair) {
  ...
  let [cardType, cardStatus] = pair.split(":");
  cardTypeToStatus[cardType] = cardStatus;
});
Transform.mapCardTypeToStatus = function (cardType) {
  return cardTypeToStatus[cardType];
};
...
function mapChecklistToIssueMkdn(checklistWithIssues) {
  let tasks = checklistWithIssues.checkItems.map(function (checkItem) {
    ...
    if (!issue) {
      // the issue may be _inactive and therefore not have been populated
      return "";
    }
...
Transform.customizeTrelloCard = function (card, includeInactive) {
  ...
  card._trelloCardStatus = Transform.mapCardTypeToStatus(card._trelloCardType);
  if (!card._trelloCardStatus) {
    // skip cards that are on lists we don't use (or otherwise can't transcribe)
    card._inactive = true;
    return;
  }
  ...
  if (_rawFallbackOwner) {
    ...
    if (!_fallbackOwner) {
      ...
      _fallbackOwner = trelloBoard.members.find(function (member) {
        return (
          member.username.toLowerCase() === _rawFallbackOwner.toLowerCase()
        );
      });
    }
  }
